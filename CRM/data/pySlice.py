...
Slic3r = "..\\slic3r\\Slic3r-console.exe"
...
def calc_lenth(filename):
        gcode = GCode(open(filename, "rU"))
        return(gcode.filament_length)

def calc_length_stl(stl_file_name):
        data = Slic3r + ' ' + stl_file_name
        out_f_name = stl_file_name.replace('stl','gcode')
        print(out_f_name)
        os.system(data)
        return calc_lenth('{}'.format(out_f_name))

def calc_length_stl_material(stl_file_name, material_id = 1) :
        Materials_Cost = {1:500,2:300,3:750}
        return round(calc_length_stl(stl_file_name)*(Materials_Cost.get(material_id)/5000)*1.65, 2)
